...
public class UART{
        ...
        synchronized(device){
            ...
            if (ret <= 0) {
                while (true) {
                    ...
                    LogUtil.i(TAG,"============222222222222111111111==========="+ret);
                   if ((device.event & uart_mask) != 0) {
                        ret = read(device, uart, data, offset, len);
                        LogUtil.i(TAG,"============3333333333==========="+ret);
                        if (ret == 0) {
    ...
    public static int fixedWrite(Device device, int uart, byte[] data, int offset, int len){
        int size = 0;
        LogUtil.i(TAG,"fixedWrite    START ===LEN="+len);
        while(len > 0){
            ...
            //LogUtil.i(TAG,"fixedWrite    START ===ret="+ret);
            size += ret;
            offset += ret;
            len -= ret;
        }
