...
public class EscCommand {
		...
		if (bitmap != null) {
			...
			if (n > 0) {
				...
				for (int i = 0; i < n; i++) {
					...
					byte[] src = bitmap2Pix(grayBitmap,grayBitmap.getWidth(),grayBitmap.getHeight());
	...
	public static byte[] bitmap2Pix(Bitmap bitmap,int width, int height){
		int temp = 0;
		int m = 0;
		...
		for (int i = 0 ; i < width; i++){
			for (int y = 0; y < height/8; y++){
				for (int z = 0 ; z < 8 ; z++){
					...
					LogUtil.i("test","y=========="+temp);
					LogUtil.i("test","height=========="+height);
					int color = bitmap.getPixel(i,temp);
	...
	public void addRastBitImageParams(Bitmap bitmap){
		...
		if (bitmap != null) {
			...
			byte[] src = bitmap2Pix(outBitmap,outBitmap.getHeight(),outBitmap.getWidth());
			add(src);
