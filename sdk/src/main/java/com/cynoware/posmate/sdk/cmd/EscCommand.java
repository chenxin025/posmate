...
package com.cynoware.posmate.sdk.cmd;
...
import com.cynoware.posmate.sdk.util.GpUtils;

import java.util.ArrayList;

import static com.cynoware.posmate.sdk.printer.Printer.SELECT_CENTER_MODE;
import static com.cynoware.posmate.sdk.printer.Printer.SELECT_LEFT_MODE;
import static com.cynoware.posmate.sdk.printer.Printer.SELECT_RIGHT_MODE;

public class EscCommand {

	private ArrayList<byte[]> mCmdList = new ArrayList<byte[]>();

	public class WrongParaException extends Exception {
		private static final long serialVersionUID = 1L;
	}

	public class ExceedBmpSizeException extends Exception {
	...
	private Bitmap setBitmapScale(Bitmap bitmap, int width, int height){
		Matrix matrix = new Matrix();
		float scaleWidth = (float) (188f / width);
		float scaleHeight = (float)(32f/height);

		matrix.postScale(scaleWidth, scaleHeight);
		Bitmap resizeBmp2 = Bitmap.createBitmap(bitmap,0,0,bitmap.getWidth(),
				bitmap.getHeight(),matrix,true);
		return resizeBmp2;
	}

	private int getXlWithNV(int width){
		return width / 8 + width % 8;
	}

	private int getYlWithNV(int height){
	...
	public void addDownloadNvBitImage(Bitmap[] bitmap) {
		if (bitmap != null) {
			...
			if (n > 0) {
				...
				int xlTemp,ylTtemp;
				xlTemp = getXlWithNV(bitmap[0].getWidth());
				ylTtemp = getYlWithNV(bitmap[0].getHeight());
				if (xlTemp > 127 || ylTtemp > 127){
					return;
				}
				byte xl = (byte)xlTemp;
				byte yl = (byte)ylTtemp;
				...
				for (int i = 0; i < n; i++) {
					...
					//Bitmap outBitmap = setBitmapScale(grayBitmap,grayBitmap.getWidth(),grayBitmap.getHeight());
					byte[] src = bitmap2Pix(grayBitmap,grayBitmap.getWidth(),grayBitmap.getHeight(),0);
	...
	public static byte[] bitmap2Pix(Bitmap bitmap,int width, int height, int type){
		int temp = 0;
		int m = 0;
		...
		for (int i = 0 ; i < width; i++){
			for (int y = 0; y < height/8; y++){
				for (int z = 0 ; z < 8 ; z++){
					...
					int color = 0;
					if (type == 0){
						color = bitmap.getPixel(i,temp);
					}else if (type == 1){
						color = bitmap.getPixel(temp,i);
					}
	...
	public void addRastBitImageParams(Bitmap bitmap){
		//TODO

		if (bitmap != null) {

			int xlTemp,ylTtemp;
			xlTemp = bitmap.getWidth();//getXlWithNV(bitmap.getWidth());
			ylTtemp = bitmap.getHeight();//getYlWithNV(bitmap.getHeight());
			//if (xlTemp > 127 || ylTtemp > 127){
			//	return;
			//}
			byte xl = (byte)(xlTemp/8);
			byte yl = (byte)ylTtemp;
				//byte[] command = new byte[3];
				...
				add(command);
			Bitmap outBitmap = setBitmapScale(bitmap,bitmap.getWidth(),bitmap.getHeight());
			byte[] src = bitmap2Pix(bitmap,bitmap.getHeight(),bitmap.getWidth(),1);
			add(src);
	...
	public void addSelectMode(int type){
		byte[] cmd = new byte[3];
		cmd[0] = 0x1b;
		cmd[1] = 0x61;

		switch (type){
			case SELECT_LEFT_MODE:
				cmd[2] = 0x00;
				break;
			case SELECT_CENTER_MODE:
				cmd[2] = 0x01;
				break;
			case SELECT_RIGHT_MODE:
				cmd[2] = 0x02;
				break;
			default:
				break;
	...
	public void addSetTabChar(byte[] selectPos){
		int len = selectPos.length;
		byte[] cmd = new byte[3+len];
		cmd[0] = 0x1b;
		cmd[1] = 0x41;
		for (int i = 0; i < len; i++){
			cmd[2+i] = selectPos[i];
