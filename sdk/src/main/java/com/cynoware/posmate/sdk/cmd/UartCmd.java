...
import com.cynoware.posmate.sdk.io.Event;
import com.cynoware.posmate.sdk.service.Buf;
...
public class UartCmd {
    ...
    private static final String TAG = "UartCmd";
	...
	private static int CMD_CHECKSUM(BleDevice bleDevice) {
		return Utils.byte2short_le(bleDevice.uartCmdOut, 2);
	}

    private static int uartCheckSum(byte[] buf, int offset, int size){
        int res = Utils.cksum(buf, offset, size);
        return (res & 0xffff);
    }
