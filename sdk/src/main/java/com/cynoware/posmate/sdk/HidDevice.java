			...
			if (mEventThread == null) {
				...
				mEventThread = new Thread() {
					public void run() {
						...
						//LogUtil.i(TAG,"$$$$$$$$$$$$$$$$$$$$$$$$"+Thread.dumpStack());
						LogUtil.i(TAG, "====================> getCloseFlag()"+getCloseFlag()+ "     isConnected()= "+isConnected() );
						while (!getCloseFlag() && isConnected() ) {
							...
							int size = -1;
							LogUtil.i(TAG,"**************usb event 11****************");
							if ( mUsbHandler.mRequestEvent.queue(buffer, buffer.capacity())) {
								LogUtil.i(TAG,"**************usb event 22****************");
								UsbRequest res = mUsbHandler.mDevConn.requestWait();
								LogUtil.i(TAG,"**************usb event 33****************");
